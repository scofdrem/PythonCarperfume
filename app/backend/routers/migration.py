...
@router.post("/fix-site-content-content-value")
async def fix_site_content_content_value(
    db: AsyncSession = Depends(get_db),
):
    """One-time migration: add content_value column if missing, populate defaults for NULLs, make NOT NULL"""
    try:
        # Step 1: Add column as nullable if it doesn't exist
        await db.execute(text(
            "ALTER TABLE site_content ADD COLUMN IF NOT EXISTS content_value VARCHAR DEFAULT '{}'"
        ))
        await db.commit()
        logger.info("Migration step 1: added content_value column (nullable with default)")

        # Step 2: Set content_value='{}' for all rows where it is NULL
        result = await db.execute(
            text("UPDATE site_content SET content_value = '{}' WHERE content_value IS NULL")
        )
        await db.commit()
        rows_updated = result.rowcount
        logger.info(f"Migration step 2: updated {rows_updated} rows with content_value='{{}}'")
        ...
        await db.commit()
        logger.info("Migration step 3: set content_value as NOT NULL")

        return {"status": "success", "rows_updated": rows_updated, "message": "content_value column added and populated"}
    except Exception as e:
        await db.rollback()
        logger.error(f"Migration error: {str(e)}", exc_info=True)
        return {"status": "error", "detail": str(e)}